...
def possible_row_filtering(code):
    lines = code.splitlines()
    for line in lines:
        if "print" not in line and ("==" in line or "!=" in line or ">" in line or "<" in line or "iloc" in line):
            return True
    return False


# Collecting the pipeline details and extract the target column based on file data structure.
...
def main(test_mode=False):
    """
    Fetch all the pipeline details from corpus and parse it using
    libcst library and fetch the target, dropped, renamed column names
    and create static_info.json file.
    """
